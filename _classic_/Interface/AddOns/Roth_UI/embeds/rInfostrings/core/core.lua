...
if not cfg.embeds.rInfoStrings then return end
  ...
  local rsiTimeLevel = 0
  local rsiTimeTotal = 0
  local lastCheckCount = 120
  ...
  local f4 = CreateFrame("Frame", "rInfoStringsContainer4", frame)
  f4:RegisterEvent("TIME_PLAYED_MSG")
  ...
  f4:SetScript("OnEvent", function(self, event, arg1, arg2, ...)
    if (event == "TIME_PLAYED_MSG") then
      rsiTimeTotal = disp_time(arg1)
      rsiTimeLevel = disp_time(arg2)
    end
  end)
  ...
  f4.text = rsiCreateFontString(f4,11)
  ...
  local function rsiUpdateStrings()
    ...
    local f4Text = ""
    f4Text = "Level Time: " ..rsiTimeLevel.."\n Total Time: "..rsiTimeTotal

    f4.text:SetText(f4Text or "")
    f4:SetHeight(f4.text:GetStringHeight())
    f4:SetWidth(f4.text:GetStringWidth())
  end
